...
Dokumen ini merupakan laporan teknis lengkap proses pengolahan data *time series* konsentrasi Nitrogen Dioksida ($\text{NO}_2$) dari berkas CSV `jabon_NO2.csv` untuk wilayah **Kecamatan Jabon, Kabupaten Sidoarjo** (periode **31 Agustus 2025 s.d. 31 Agustus 2026**). 
...
Untuk mengelola seluruh data kualitas udara di **Kecamatan Jabon, Kabupaten Sidoarjo**, dirancang arsitektur basis data relasional berjenjang (*multi-tier schema*) di cloud database **PostgreSQL Aiven**. Struktur ini dirancang secara modular agar mampu menampung:
...
3. **Tabel Spesifik Parameter (`jabon_no2`)**: Disediakan khusus untuk kemudahan koneksi langsung ke node KNIME Analytics Platform dalam menganalisis parameter target utama.
...
│   Tabel: jabon_pollutants_raw     │   Tabel: jabon_pollutants_clean                    │
...
├───────────────────────────────────┴────────────────────────────────────────────────────┤
│   3. Tabel Target Analisis KNIME: jabon_no2                                            │
...
-- ====================================================================
DROP TABLE IF EXISTS jabon_pollutants_raw CASCADE;

CREATE TABLE jabon_pollutants_raw (
...
-- ====================================================================
DROP TABLE IF EXISTS jabon_pollutants_clean CASCADE;

CREATE TABLE jabon_pollutants_clean (
    id BIGSERIAL PRIMARY KEY,
    date DATE NOT NULL UNIQUE,
...
-- ====================================================================
DROP TABLE IF EXISTS jabon_no2 CASCADE;
...
FROM jabon_pollutants_raw r
JOIN jabon_pollutants_clean c ON r.date = c.date
ORDER BY r.date ASC;
```

---

### D. Skrip Otomasi Ingestion Python (`ingest_aiven_jabon.py`)

Skrip berikut secara otomatis mengunggah tabel multi-polutan mentah, tabel multi-polutan bersih, serta tabel spesifik `jabon_no2` ke instance database PostgreSQL Aiven. Anda dapat menyalin kode di bawah atau mengunduh berkas skrip langsung melalui tombol berikut:

<a href="ingest_aiven_jabon.py" download="ingest_aiven_jabon.py" style="display:inline-block;padding:10px 22px;margin:8px 0 16px 0;background:#1a73e8;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:bold;font-size:14px;box-shadow:0 2px 6px rgba(26,115,232,0.4);">📥 Unduh Script ingest_aiven_jabon.py</a>
...
> **Panduan Gambar 01 (`data/images/analisis_no2/01-aiven-postgres-dashboard.png`):** Tangkapan layar dari dashboard Aiven Console yang menampilkan status service PostgreSQL dalam kondisi *Running*, lengkap dengan informasi Host URI, Port, dan Database Name.

### Cara Menjalankan Script Python:
* Jalankan skrip ingestion melalui terminal sistem operasi:
  ```bash
  python ingest_aiven_jabon.py
  ```

![Log Eksekusi Ingest Python](../data/images/analisis_no2/02-python-ingestion-log.png)
> **Panduan Gambar 02 (`data/images/analisis_no2/02-python-ingestion-log.png`):** Tangkapan layar terminal yang menampilkan log sukses pengunggahan tabel mentah, tabel bersih, dan tabel analisis `jabon_no2`.
...
Node ini menerima koneksi DB (port berwarna merah) dari `PostgreSQL Connector` untuk mengidentifikasi dan memilih tabel data Kecamatan Jabon.
...
* **Langkah Konfigurasi**:
  ...
  3. Pilih skema `public` dan tentukan nama tabel `jabon_no2`.
...
* **Langkah Konfigurasi**:
  ...
  2. Tulis kueri SQL pada editor, misalnya:
     ```sql
     SELECT * FROM jabon_no2 WHERE no2 IS NOT NULL;
     ```
...
| **DB Table Selector** | Menerima koneksi DB (merah) dari Connector untuk memilih tabel. | • Database Schema (`public`)<br>• Target Table Name (`jabon_no2`) |
...
## 3. Penjelasan Fitur, Rumus, dan Perhitungan Statistik Riil Jabon ($N = 240$)
...
> **Panduan Gambar 09 (`data/images/analisis_no2/09-knime-statistics-table-output.png`):** Tangkapan layar tampilan tabel hasil eksekusi node `Statistics` KNIME untuk kolom `no2` data riil Kecamatan Jabon (Missing: 126, Overall Sum: 0.01, Skewness: 0.732, Kurtosis: 2.848).

---

### A. Total Data ($N$) & Missing Value Count

* **Penjelasan**: Dari 366 hari kalender observasi satelit di Kecamatan Jabon, terdapat 126 baris bernilai kosong (`null`). Perhitungan statistik deskriptif secara otomatis mengabaikan nilai `null` sehingga total sampel data valid yang diproses adalah $N = 240$.
* **Rumus**:
...
* **Penjelasan**: Nilai konsentrasi $\text{NO}_2$ terkecil dan terbesar di antara 240 data valid Kecamatan Jabon.
* **Rumus**:
...
* **Penjelasan**: **Overall Sum** adalah jumlah akumulasi dari 240 sampel data valid, sedangkan **Mean** adalah nilai rata-rata konsentrasi $\text{NO}_2$ di Kecamatan Jabon.
* **Rumus**:
...
## 4. Tabel Ringkasan Komparasi Statistik Riil Jabon ($N = 240$)
...
Praktikum integrasi PostgreSQL Aiven dan KNIME Analytics Platform untuk data $\text{NO}_2$ Kecamatan Jabon membuktikan bahwa:
